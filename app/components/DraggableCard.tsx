...
export default function DraggableCard({ course, compact, searchQuery, customSubjects, schedule, assignmentRows, readOnly = false }: DraggableCardProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes}
      className={`relative w-full max-h-24 overflow-hidden rounded border-[1.5px] ${colors.border} border-l-2 ${colors.borderLeft} ${colors.bg} ${compactClasses} cursor-grab active:cursor-grabbing hover:shadow transition-all shadow-sm ${isDragging && isCtrlPressed ? 'ring-2 ring-blue-400 bg-blue-50' : ''}`}>
