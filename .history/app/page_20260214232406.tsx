    ...
    >
      ...
      <div className="min-h-[1.75rem] px-1 py-0.5 flex items-center bg-white overflow-hidden">
        <span className="text-xs font-bold text-red-600 text-left w-full whitespace-normal break-words leading-tight pl-1">{(() => {
          // Afficher le professeur, en filtrant les valeurs vides/invalides
          if (!course.teacher || course.teacher === 'Non assigné' || course.teacher === '' || course.teacher === '?') {
            return '?';
          }
          // Retirer les "/" et les "?" du professeur affiché
          const cleanedTeacher = course.teacher
            .split('/')
            .map((t: string) => t.trim())
            .filter((t: string) => t && t !== '?')[0]; // Prendre le premier valide
